...
When you run `agentforge init`, the generated `Dockerfile` pulls the `agentforge` binary from the published container image at `ghcr.io/agent-forge/agent-forge:latest` (via `COPY --from`), bundles it with your compiled databases and config, and runs `agentforge serve` as the entrypoint. No build toolchain is needed inside the agent image — it's just Alpine + the binary + your data (~50 MB total).
...
The generated `Dockerfile` uses `COPY --from=ghcr.io/agent-forge/agent-forge:latest` to pull the pre-built `agentforge` binary from the published container image. It then bundles your compiled databases (`data/memory.chromem/`, `data/knowledge.cayley/`) and `agent.yaml`, and sets the entrypoint to `agentforge serve`. No Go toolchain or source code is needed — the agent image is just Alpine Linux + the binary + your data.
...
2. Build and push a multi-arch Docker image (`linux/amd64`, `linux/arm64`) to GitHub Container Registry
...
- `ghcr.io/agent-forge/agent-forge:latest`
- `ghcr.io/agent-forge/agent-forge:<version>`

This Docker image is referenced by agent Dockerfiles (generated by `agentforge init`) to pull the `agentforge` binary without needing Go or source code.
