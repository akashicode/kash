...
When you run `agentforge init`, the generated `Dockerfile` downloads the `agentforge` binary from [GitHub Releases](https://github.com/agent-forge/agent-forge/releases/latest) during `docker build`, bundles it with your compiled databases and config, and runs `agentforge serve` as the entrypoint. Docker's `TARGETARCH` auto-resolves for multi-arch builds (`amd64`/`arm64`). No build toolchain is needed inside the agent image — it's just Alpine + the binary + your data (~50 MB total).

You can also download the binary directly from GitHub Releases to use as a local CLI tool — just add it to your `PATH`.
...
The generated `Dockerfile` downloads the `agentforge` binary from [GitHub Releases](https://github.com/agent-forge/agent-forge/releases/latest) during `docker build`. It uses Docker's `TARGETARCH` build arg to automatically fetch the correct architecture (`amd64`/`arm64`). It then bundles your compiled databases (`data/memory.chromem/`, `data/knowledge.cayley/`) and `agent.yaml`, and sets the entrypoint to `agentforge serve`. No Go toolchain or source code is needed — the agent image is just Alpine Linux + the binary + your data.
...
Releases are automated via GitHub Actions. Push a version tag and the workflow builds CLI binaries for all 5 platforms and publishes them to GitHub Releases:
...
These binaries serve dual purpose:
1. **Local CLI usage** — download, add to `PATH`, and use `agentforge init`, `agentforge build`, etc. directly on your machine
2. **Agent Docker images** — the generated `Dockerfile` (from `agentforge init`) automatically downloads the latest binary from GitHub Releases during `docker build`
